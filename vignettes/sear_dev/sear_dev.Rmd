...
```{r}
ConFile <- system.file("data-raw", "reduced", "DATA_20220705_105732.txt", package = "sear", mustWork = T)
...
TODO: Create a processing pipline for the hydroball WISE-Man 2019 setup.
...
MainLog <- MainLog %>% mutate(
  SyntSBE19 = data_synthesis_any_near(DateTime, SBE19$DateTime),
  SynHOCR = data_synthesis_any_near(DateTime, HOCRTimeIndex),
)
...
* Maintaining data consistency through the interaction of the human and the computer.
...
while (T){
  ...
  if (length(MainLog$DateTime) < i+1){
  ...
  x = MainLog$DateTime[i]
  y = MainLog$DateTime[j+1]
  ...
  while (interval(x,y) < seconds(3)){
    ...
    if (length(MainLog$DateTime) < j+1){
      print("This is the end")
      break
    }
    
    j = j+1
    y = MainLog$DateTime[j]
  }
