...
When parsed data is loaded in the app, a selection display column in the dashboard body is populated. This selection display is composed of a map which expose the discrete (aggregated to second ?) data point with the observation type, of several filters (speed, time, need to add pitch and roll when available) and of a polar plot exposing the azimuth of point selected on the map. 
...
Apla <- Apla %>% filter(DateTime %within% Station1PaP)
