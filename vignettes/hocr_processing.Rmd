...
# Python function
RawHOCR <- reticulate::py_to_r(Hocr$from_file(ConFile))

# With reticulate, if a python custom class object is returned then a reference to that object is returned
# Object reference are not persistent therefore we cannot save such an reference list ...
# It return null pointer.
RawHOCR <- purrr::map(RawHOCR$packets, ~ list(
  "channel" = .$channel,
  "checksum" = .$checksum,
