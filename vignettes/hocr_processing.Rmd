...
BinFile <- system.file("extdata", "DATA_20211111_171729.bin", package = "sear", mustWork = T)
...
The HOCR data is quite large and mainly compose of bad data, aquiered in transit, that we don´t want to loose time to process. Therfore, only the selected point designing a transect or a station need to enter the processing chain. For that we use filters on the `RawHOCR` list of HOCR packets.
...
Station1PaP <- interval(ymd_hms("2021-11-11 18:49:16"), ymd_hms("2021-11-11 18:49:25"))
...
CalList <- purrr::map(CalFiles, read_cal)
...
*The dark offset for HOCR is disregarded*
...
DarkGlobCal <- DarkRawData %>% group_by(Instrument, SN) %>% nest(RawData = !matches("Instrument|SN"))
...
Find the center time of obs interval and select nearest dark values.
```{r}
...
Now that we have the data interpolated to common time co-ordinate and corrected for dark offset we can tranform it back to long format. For convenience and as we can trace back the physical variable (ES or LU) back to the instrument `SN`, we use the term `Channels` (as in HOCR packets) to name the variables
