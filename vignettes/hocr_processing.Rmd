...
To harmonize the visual identity of the app we defne global diplay parameter for plotly graph.
...
### QC data table for HOCR L1b -> L2

This data table is used to display the quality check table. At beginning of L1b all data is considered good `QC = 1`, the user can then select spectrum from the plot and mark it as bad `QC = 0` or suspicious `QC = 2`. 

```{r}
HOCRdata <- HOCRdata %>%
  filter(str_detect(Instrument, "HSE|HPL")) %>%
  mutate(DataTable = purrr::map(CalData, ~ unique(select(.x, !all_of(c("Channels","Wavelength","FieldLength","DataType","CalLines","FitType","a0","a1","im","cint"))))))
```
