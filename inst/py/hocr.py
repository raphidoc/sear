...
class Hocr(KaitaiStruct):
    ...
    def _read(self):
        ...
        while not self._io.is_eof():
            self.packets.append(Hocr.HocrPacket(self._io, self, self._root))
            i += 1
    ...
    class HocrPacket(KaitaiStruct):
        ...
        def _read(self):
            self.gpstime = self._io.read_u4be()
            ...
            self.channel = [None] * (180)
            for i in range(180):
                self.channel[i] = self._io.read_u2be()
